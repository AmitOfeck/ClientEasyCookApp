...
import { updateDish, getDishById } from "../services/dish_service";
import { Cuisine, Level, Limitation, IDish } from "../services/intefaces/dish";

const DishUpdateScreen: React.FC<{ navigation: any, route: any }> = ({ navigation, route }) => {
    const { dishId } = route.params; // Dish ID passed via navigation

    const [dishName, setDishName] = useState("");
    const [dishPrice, setDishPrice] = useState("");
    const [dishCuisine, setDishCuisine] = useState<string | null>(null);
    const [dishLimitation, setDishLimitation] = useState<string | null>(null);
    const [dishDifficulty, setDishDifficulty] = useState<string | null>(null);
    ...
    useEffect(() => {
        const fetchDishDetails = async () => {
            try {
                ...
                setDishCuisine(dish.cuisine);
                setDishLimitation(dish.limitation);
                setDishDifficulty(dish.level);
                setDishDescription(dish.details || "");
                setDishCalories(dish.dishCalories.toString());
                setIngredientsCost(dish.ingredientsCost.toString());
                setAverageDishCost(dish.averageDishCost.toString());
    ...
    const handleUpdateDish = async () => {
        ...
        try {
            ...
            const { request, abort } = updateDish(dishId, dishData);
            const response = await request;
            ...
            Alert.alert("Error", "Failed to update the dish. Please try again.");
        }
    };
    ...
    return (
        <View style={styles.container}>
            <ScrollView contentContainerStyle={[styles.scrollViewContent, { paddingBottom: 80 }]}>
                ...
                <Text style={styles.headerText}>Update Dish</Text>
                ...
                <TouchableOpacity style={styles.createButton} onPress={handleUpdateDish}>
                    <Text style={styles.createButtonText}>Update Dish</Text>
                </TouchableOpacity>
            </ScrollView>
